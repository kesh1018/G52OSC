...
public class SJF {
	...
	public static void main(String[] args) {
		...
		int n;
		int waitingTime[], burstTime[], turnaroundTime[], process[];
		...
		n = num.nextInt();				//input the number of process
		...
		for(int i = 0; i < n; i++){
			for(int j=0;j<n-1;j++){ 	
				if(burstTime[j] > burstTime[j+1]){  //compare processess's burst time,if previous are more than next,swap
					...
					temp = waitingTime[j]; 					//allocate the waiting time for each process  
					waitingTime[j] = waitingTime[j+1]; 		//according to burst time
		...
		for(int i = 0; i < n; i++){
			System.out.println("      "+ process[i] +" \t"+burstTime[i]+"\t     "+waitingTime[i]+"\t\t    "+turnaroundTime[i]);
		}
